...
import { uploadToCloudinary, deleteFromCloudinary, cloudinary, } from "../middlewares/cloudinary.js";
import fs from "fs";
/**
 * @swagger
 * /api/products:
...
export const createProduct = async (req, res) => {
    try {
        ...
        if (file) {
            try {
                const uploadResult = await uploadToCloudinary(file, "mini-ecommerce/products");
    ...
    try {
        ...
        if (cartItems.length > 0) {
            return res.status(400).json({
                message: "Cannot delete product that is in users' carts. Remove from carts first.",
            });
        }
